...
import com.microsorvices.demo.elastic.query.service.Constants;
...
public class MastodonQueryUserJwtConverter implements Converter<Jwt, AbstractAuthenticationToken> {
    ...
    public MastodonQueryUserJwtConverter(MastodonQueryUserDetailsService userDetailsService) {
    ...
    public AbstractAuthenticationToken convert(Jwt jwt) {
        ...
        return Optional.ofNullable(
                userDetailsService.loadUserByUsername(jwt.getClaimAsString(USERNAME_CLAIM))
        ) .map(userDetails -> {
                    ((MastodonQueryUser) userDetails).setAuthorities(authoritiesFromJwt);
    ...
    private Collection<String> getRoles(Jwt jwt) {
        ...
        if (roles instanceof Collection) {
            return ((Collection<String>) roles).stream().map(authority ->
                    DEFAULT_ROLE_PREFIX + authority.toUpperCase()
            ).collect(Collectors.toList());
        }
